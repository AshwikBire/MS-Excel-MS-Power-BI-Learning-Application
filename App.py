...
with TABS[2]: st.markdown("### Excel Functions — Hands‑on Lab") left, right = st.columns([0.55, 0.45]) with left: card( "Lookup Playground (INDEX/MATCH style)", body="Use the selector to lookup a person's revenue in a chosen month.") target_month = st.selectbox("Select Month", months, index=0) target_person = st.selectbox("Select Person", sorted(people)) subset = sales_df[(sales_df["